...
`.ts`, `.mts` and `.cts`, as TypeScript 5.9: everything
...
`import("m").T` and type predicates. JSX is not read yet; a `.tsx` file is
not this package's.
...
[evidence](docs/evidence/corpus-typescript-lib-dts.json).) A declaration
